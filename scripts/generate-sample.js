...
const formatList = (items, bullet = "-") => items.map((entry) => `${bullet} ${entry}`).join("\n");

const sentenceFromValues = (values) =>
  values.length ? values.map((value) => value.replace(/_/g, " ")).join(", ") : "the chosen core values";

const describeMood = (config) => {
  const segments = [];
  if (config.mood?.default_mood?.length) {
    segments.push(`Default mood anchors: ${config.mood.default_mood.map((mood) => `**${mood}**`).join(" & ")}.`);
  }
  ...
  if (config.mood?.when_criticized) {
    segments.push(`When criticized, respond in a **${config.mood.when_criticized.replace(/_/g, " ")}** manner.`);
  }
  return segments.join(" ");
};

const describeVoice = (config) => {
  const { voice } = config;
...
const describePersonalization = (config) => {
...
const describeRedLines = (config) =>
  config.red_lines
    .map((line) => line.replace(/_/g, " "))
    .map((line) => `Honor the rule: **${line}**.`);

const describeAdvanced = (config) => {
  const blocks = [];
...
const flirtinessGuidance = (config) => {
...
const formatFewShots = (config, includeFewShots) => {
...
const buildSystemPrompt = (config, options = {}) => {
...
const systemPrompt = buildSystemPrompt(defaultConfig, { includeFewShots: false });
const cheatsheet = buildCheatsheet(defaultConfig);
const personaJson = JSON.stringify(defaultConfig, null, 2);
...
fs.writeFileSync(path.join(samplesDir, "quick-start-system-prompt.md"), systemPrompt, "utf8");
fs.writeFileSync(path.join(samplesDir, "quick-start-config.json"), personaJson, "utf8");
fs.writeFileSync(path.join(samplesDir, "quick-start-cheatsheet.md"), cheatsheet, "utf8");

console.log("Sample files generated in /samples");
